...
-(void)lsqInitView;
{
    ...
    [_recordButton setCenter:CGPointMake(rect.size.width/2, rect.size.height/2)];
    [_recordButton setImage:[UIImage imageNamed:@"photograph_Select"] forState:UIControlStateNormal];
    [_recordButton setImage:[UIImage imageNamed:@"photograph_Select"] forState:UIControlStateSelected];
    _recordButton.adjustsImageWhenHighlighted = NO;
    [self addSubview:_recordButton];
    
    CGFloat albumBtnCenterY = rect.size.height/2 ;
    CGFloat albumLabCenterY = albumBtnCenterY + buttonWidth/2 + labelHeight/2 ;
    
    // 后退按钮
    _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0 , 0, buttonWidth, buttonWidth)];
    [_cancelButton setCenter:CGPointMake(sideDistance + buttonWidth/2, albumBtnCenterY)];
    ...
    [_cancelLabel setCenter:CGPointMake(_cancelButton.center.x, albumLabCenterY)];
    _cancelLabel.font = [UIFont systemFontOfSize:12];
    _cancelLabel.textColor = [UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:1];
    _cancelLabel.text =  NSLocalizedString(@"return", @"后退");
    _cancelLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:_cancelLabel];
    
    // 查看按钮
    _checkVideoButton = [[UIButton alloc]initWithFrame:CGRectMake(0 , 0, buttonWidth, buttonWidth)];
    [_checkVideoButton setCenter:CGPointMake(rect.size.width - sideDistance - buttonWidth/2 - sideDistance - buttonWidth/2, albumBtnCenterY)];
    [_checkVideoButton setImage:[UIImage imageNamed:@"style_default_crop_btn_record"] forState:UIControlStateDisabled];
    [_checkVideoButton setImage:[UIImage imageNamed:@"style_default_camera_record_btn_record"] forState:UIControlStateSelected];
    _checkVideoButton.enabled = NO;
    [_checkVideoButton addTarget:self action:@selector(clickBtn:) forControlEvents:UIControlEventTouchUpInside];
    _checkVideoButton.adjustsImageWhenHighlighted = NO;
    [self addSubview:_checkVideoButton];
    
    _checkVideoLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, labelWidth, labelHeight)];
    [_checkVideoLabel setCenter:CGPointMake(_checkVideoButton.center.x, albumLabCenterY)];
    _checkVideoLabel.font = [UIFont systemFontOfSize:12];
    _checkVideoLabel.textColor = [UIColor colorWithRed:0.7 green:0.7 blue:0.7 alpha:1];
    _checkVideoLabel.text =  NSLocalizedString(@"check", @"查看");
    _checkVideoLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:_checkVideoLabel];
    
    // 确认按钮
    _completeButton = [[UIButton alloc]initWithFrame:CGRectMake(0 , 0, buttonWidth, buttonWidth)];
    [_completeButton setCenter:CGPointMake(rect.size.width - sideDistance - buttonWidth/2, albumBtnCenterY)];
    ...
    [_completeLabel setCenter:CGPointMake(_completeButton.center.x, albumLabCenterY)];
    ...
    _completeLabel.text =  NSLocalizedString(@"videoOK", @"上传");
...
- (void)enabledBtnWithComplete:(BOOL)enabledComplete
{
    ...
    if (enabledComplete) {
        ...
        _checkVideoButton.selected = YES;
        _checkVideoButton.enabled = YES;
        _checkVideoLabel.textColor = HEXCOLOR(0x22bbf4);
